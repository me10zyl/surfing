...
public class HighAvaliabilityProxyProviderTest {
    public static void main(String[] args) {
        ...
        SurfSpider.create().setRequests(Arrays.asList(request, request2)).setProxyProvider(new HighAvailabilityProxyProvider(Arrays.asList(proxies))).setProcessor(new SurfPageProcessor() {
            @Override
